...
def nth_order_isothermal(x,y):
    ...
    D(d2C/dr2) = k*C
    """
    return np.vstack((y[1], parameter.phi2*y[0]**parameter.n ))


"""
First order isothermal reaction 1D slab
"""
def nth_order_isothermal_phi(x,y):
    """
    d2y/du2 = -phi2*y
    """
    return np.vstack((y[1], parameter.phi2*y[0]**parameter.n ))
...
def nth_order_non_isothermal(x,y):
    ...
    return np.vstack((y[1],
                      parameter.phi2*np.exp(-parameter.Ea/parameter.R/y[2])*y[0]**parameter.n,
                      y[3],
                      parameter.phi2*(-parameter.delH)*np.exp(-parameter.Ea/parameter.R/y[2])*y[0]))
